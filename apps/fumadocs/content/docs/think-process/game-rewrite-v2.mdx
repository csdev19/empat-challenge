...
Game Flow
	1.	The SLP starts the game during a therapy session
	2.	The system displays:
	•	A target word (e.g. “DOG”)
	•	A set of picture options (e.g. dog, cat, bird)
	3.	It is the student’s turn
	4.	The student selects one option
	5.	The system evaluates the selection:
	•	Correct → success
	•	Incorrect → error
	6.	The result is:
	•	Recorded as a trial
	•	Shown immediately to the SLP
	7.	The turn ends
	8.	The SLP advances to the next word
